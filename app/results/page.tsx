...
export default function ResultsPage() {
  ...
  useEffect(() => {
    const raw = sessionStorage.getItem('scenario-result');
    if (raw) {
      try {
        setData(JSON.parse(raw));
      } catch {
        setError('Failed to parse scenario result. Please run the scenario again.');
      }
    } else {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ...
      { root: null, rootMargin: '-20% 0px -40% 0px', threshold: [0, 0.1, 0.25, 0.5, 0.75, 1] }
    );
  ...
  const handleRerun = useCallback(async () => {
    ...
    try {
      ...
      sessionStorage.setItem('scenario-result', JSON.stringify(json));
      setData(json);
      setError(null);
  ...
  const hostToY = Object.fromEntries(hostIds.map((id, index) => [id, hostIds.length - 1 - index]));
  const yToHost = Object.fromEntries(hostIds.map((id, index) => [hostIds.length - 1 - index, id]));
  ...
  if (data?.events && data.hostCategories) {
    for (const ev of data.events) {
      ...
      const point = { x: ev.timestampMs, y: yVal, hostId: ev.hostId, timestampMs: ev.timestampMs };
  ...
  if (data?.phase1Timeline && data?.phase2Timeline && data?.hostCategories) {
    ...
    const allTs = Array.from(new Set([...Array.from(p1ByTs.keys()), ...Array.from(p2ByTs.keys())])).sort((a, b) => a - b);
    for (const ts of allTs) {
      const p1Vals = p1ByTs.get(ts);
      const p2Vals = p2ByTs.get(ts);
      confidenceChartData.push({
        timestampMs: ts,
        phase1Avg: p1Vals && p1Vals.length > 0 ? p1Vals.reduce((s, v) => s + v, 0) / p1Vals.length : null,
        phase2Avg: p2Vals && p2Vals.length > 0 ? p2Vals.reduce((s, v) => s + v, 0) / p2Vals.length : null,
      });
    }
  }

  const detectedTransitionMs =
    data?.detectedTransitionMs ?? data?.detectionResults?.[0]?.detectedTransitionMs ?? null;
  const actualTransitionMs = data?.actualTransitionMs ?? data?.phase1DurationMs ?? 3_600_000;
  const detectedMin = detectedTransitionMs !== null ? detectedTransitionMs / 60000 : null;
  const actualMin = actualTransitionMs / 60000;
  const deltaMin = detectedMin !== null ? detectedMin - actualMin : null;
  ...
  return (
    <div className="relative min-h-screen bg-zinc-950 text-zinc-100 font-sans">

      {/* ── Dimmed KineticMatrix fixed background ── */}
      <div className="fixed inset-0 z-0 pointer-events-none" aria-hidden>
        <KineticMatrix
          ...
          showTitle={false}
        />
        {/* Dark overlay to dim the background significantly */}
        <div
      ...
      {/* ── All content stacks above the fixed background ── */}
      <div className="relative z-10">

        {/* ══ MAIN NAVBAR ══ */}
        <nav className="sticky top-0 z-50 flex items-center justify-between px-6 md:px-10 h-16 bg-zinc-950/80 backdrop-blur-md border-b border-zinc-800/70">
          <div className="flex items-center gap-3">
            {/* Back to landing */}
            <button
              onClick={() => router.push('/')}
              className="mr-1 p-1.5 rounded-lg text-zinc-500 hover:text-zinc-200 hover:bg-zinc-800/60 transition-all cursor-pointer"
              title="Back to home"
            >
            ...
            <span className="text-base font-bold tracking-tight text-zinc-100 select-none">PulseHide</span>
          ...
          <div className="flex items-center gap-1">
            <a
              ...
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-zinc-400 hover:text-zinc-100 hover:bg-zinc-800/60 transition-all"
            >
            ...
            <button
              onClick={handleRerun}
              disabled={loading}
              className="inline-flex items-center gap-1.5 ml-1 px-3 py-1.5 rounded-lg text-xs font-medium bg-red-600/80 hover:bg-red-500 text-white transition-all border border-red-500/40 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {loading ? (
        ...
        {/* ══ SCROLLSPY PILL NAV ══ */}
        <div className="sticky top-16 z-40 bg-zinc-950/75 backdrop-blur-md border-b border-zinc-800/70 px-6 md:px-10 py-2.5 flex items-center justify-between shadow-md">
          {/* Pill group */}
          <div className="flex items-center gap-1 bg-zinc-900/60 border border-zinc-800/60 rounded-full px-1.5 py-1">
            {[
        ...
        {/* ══ MAIN CONTENT ══ */}
        <div className="flex flex-col gap-8 px-6 md:px-10 pb-20 pt-8">

          {/* Error State */}
          {error && (
            <div className="p-4 sm:p-5 rounded-2xl bg-red-950/30 border border-red-900/60 text-red-200 text-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-lg">
              ...
              <button
                onClick={handleRerun}
                disabled={loading}
                className="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-xl bg-red-600/80 hover:bg-red-500 text-white font-medium text-xs transition-all border border-red-500/30 cursor-pointer disabled:opacity-50 self-start sm:self-auto flex-shrink-0"
              >
          ...
          {/* ── BEACON TIMELINE ── */}
          {data && (
            <div id="beacon-timeline" className="scroll-mt-32 flex flex-col gap-4 rounded-2xl bg-zinc-900/70 border border-zinc-800/80 p-6 shadow-xl backdrop-blur-sm">
              {/* Legend & Summary */}
              ...
              <div className="w-full h-[650px] pt-4">
                <ResponsiveContainer width="100%" height="100%">
                  <ScatterChart margin={{ top: 20, right: 30, bottom: 30, left: 30 }}>
                    ...
                    <XAxis
                      type="number" dataKey="x" name="Time"
                      domain={[0, totalDurationMs]} tickFormatter={formatTimeMinutes}
                      stroke="#71717a" tick={{ fill: '#a1a1aa', fontSize: 12 }}
                    />
                    <YAxis
                      type="number" dataKey="y" name="Host"
                      domain={[-0.5, 19.5]} ticks={Array.from({ length: 20 }, (_, i) => i)}
                      tickFormatter={(val) => yToHost[val] ?? ''}
                      stroke="#71717a" tick={{ fill: '#a1a1aa', fontSize: 12, fontWeight: 500 }}
                      interval={0} width={75}
                    />
                      ...
                      content={({ payload }) => {
                        ...
                        return (
                          <div className="p-3 rounded-lg bg-zinc-900 border border-zinc-700 text-zinc-100 text-xs shadow-xl space-y-1">
                            <div className="font-bold text-sm">{p.hostId}</div>
                            <div className="text-zinc-400">
                              Category:{' '}
                              <span className="font-semibold text-zinc-200 capitalize">
                                {cat === 'compromised' ? 'Compromised (C2)' :
                                  cat === 'benignRegular' ? 'Benign-Regular (Trap)' :
                                  cat === 'benignBursty' ? 'Benign-Bursty' : 'Plain Benign'}
                              </span>
                            </div>
                            <div className="text-zinc-400">
                            ...
                            <div className="text-zinc-400">
                              ...
                              <span className={`font-semibold ${p.timestampMs < phase1EndMs ? 'text-amber-400' : 'text-purple-400'}`}>
                                {p.timestampMs < phase1EndMs ? 'Phase 1 (Regular)' : 'Phase 2 (Coordinated)'}
                              </span>
                            </div>
                          </div>
                        );
                    ...
                    />
                    <ReferenceLine
                      x={phase1EndMs} stroke="#f59e0b" strokeDasharray="5 5" strokeWidth={2}
              ...
              {confidenceChartData.length > 0 && (
                <div id="confidence" className="scroll-mt-32 flex flex-col gap-3 rounded-2xl bg-zinc-900/60 border border-zinc-800/80 p-6 shadow-xl mt-2 backdrop-blur-sm">
                  {/* Title & Callout */}
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 border-b border-zinc-800/60 pb-3">
                    <div>
                      ...
                      <p className="text-xs text-zinc-500 mt-1">
                        Phase-1 and Phase-2 use independent scales — raw values are unchanged from detection.
                      </p>
                    </div>
                    {detectedMin !== null && (
                      <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-xl bg-purple-950/40 border border-purple-800/60 text-xs text-purple-200 shadow-sm self-start md:self-auto flex-shrink-0">
                        ...
                        <span>
                          ...
                          <strong className="font-mono text-purple-300">{detectedMin.toFixed(1)}m</strong> — actual was at{' '}
                          <strong className="font-mono text-zinc-200">{actualMin.toFixed(1)}m</strong>
                          {deltaMin !== null && (
                            <> (delta: <strong className="font-mono text-purple-300">{deltaSign}{deltaMin.toFixed(1)}m</strong>).</>
                          )}
                        </span>
                      </div>
                    )}
                  ...
                  <div className="w-full h-[260px] pt-2">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={confidenceChartData} margin={{ top: 10, right: 65, bottom: 20, left: 15 }}>
                        <CartesianGrid strokeDasharray="3 3" stroke="#27272a" />
                        <XAxis
                          type="number" dataKey="timestampMs" domain={[0, totalDurationMs]}
                          scale="linear" tickFormatter={formatTimeMinutes}
                          stroke="#71717a" tick={{ fill: '#a1a1aa', fontSize: 12 }}
                        />
                        <YAxis
                          yAxisId="left" orientation="left" domain={[0, 1]}
                          stroke="#f97316" tick={{ fill: '#fdba74', fontSize: 11 }}
                          tickFormatter={(v: number) => v.toFixed(1)}
                          label={{ value: 'Phase-1 Regularity', angle: -90, position: 'insideLeft', fill: '#f97316', fontSize: 11, dy: 50 }}
                          width={45}
                        />
                        <YAxis
                          yAxisId="right" orientation="right" domain={[0, 'auto']}
                          stroke="#2dd4bf" tick={{ fill: '#5eead4', fontSize: 11 }}
                          tickFormatter={(v: number) => v.toFixed(3)}
                          label={{ value: 'Phase-2 Correlation', angle: 90, position: 'insideRight', fill: '#2dd4bf', fontSize: 11, dy: 50 }}
                          width={55}
                        />
                        <Tooltip
                          content={({ payload, label }) => {
                            ...
                            return (
                              <div className="p-3 rounded-lg bg-zinc-900 border border-zinc-700 text-xs shadow-xl space-y-1">
                                <div className="text-zinc-400 font-medium">{formatTimeMinutes(label as number)}</div>
                                {payload.map((entry) => (
                                  <div key={entry.dataKey as string} className="flex items-center gap-2" style={{ color: entry.color }}>
                                    <span className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: entry.color as string }} />
                                    <span className="text-zinc-300">
                                      {entry.name}: <span className="font-mono font-semibold">{typeof entry.value === 'number' ? entry.value.toFixed(4) : '—'}</span>
                                    </span>
                                  </div>
                                ))}
                              </div>
                        ...
                        />
                        {/* Ground-truth Phase Transition (amber dashed) */}
                        <ReferenceLine
                          yAxisId="left" x={actualTransitionMs}
                          stroke="#f59e0b" strokeDasharray="5 5" strokeWidth={2}
                          label={{ value: 'Phase Transition', position: 'insideTopRight', fill: '#fbbf24', fontSize: 11, fontWeight: 600 }}
                        />
                        {/* Self-detected transition (violet dotted) */}
                        {detectedTransitionMs !== null && (
                          <ReferenceLine
                            yAxisId="left" x={detectedTransitionMs}
                            stroke="#c084fc" strokeDasharray="2 3" strokeWidth={2}
                            label={{ value: 'Self-Detected', position: 'insideBottomLeft', fill: '#d8b4fe', fontSize: 11, fontWeight: 600 }}
                          />
                        )}
                        <Line yAxisId="left" type="monotone" dataKey="phase1Avg" name="Phase-1 Signal (regularity)"
                          stroke="#f97316" strokeWidth={2} dot={false} activeDot={{ r: 4, strokeWidth: 0 }} connectNulls />
                        <Line yAxisId="right" type="monotone" dataKey="phase2Avg" name="Phase-2 Signal (correlation)"
                          stroke="#2dd4bf" strokeWidth={2} dot={false} activeDot={{ r: 4, strokeWidth: 0 }} connectNulls />
                      </LineChart>
                    </ResponsiveContainer>
                  </div>
                </div>
              ...
              {data.detectionResults && (
                <div id="results" className="scroll-mt-32 flex flex-col gap-6 rounded-2xl bg-zinc-900/60 border border-zinc-800/80 p-6 shadow-xl mt-2 backdrop-blur-sm">
                  ...
                  <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-4 p-4 rounded-xl bg-zinc-950/70 border border-zinc-800">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                        {[
                          { label: 'True Positives', value: data.evaluationResult?.truePositives?.length ?? 0, color: 'text-emerald-400' },
                          { label: 'False Positives', value: data.evaluationResult?.falsePositives?.length ?? 0, color: (data.evaluationResult?.falsePositives?.length ?? 0) > 0 ? 'text-red-400' : 'text-zinc-200' },
                          { label: 'False Negatives', value: data.evaluationResult?.falseNegatives?.length ?? 0, color: (data.evaluationResult?.falseNegatives?.length ?? 0) > 0 ? 'text-red-400' : 'text-zinc-200' },
                          { label: 'True Negatives', value: data.evaluationResult?.trueNegatives?.length ?? 0, color: 'text-zinc-200' },
                        ].map((stat) => (
                          <div key={stat.label} className="px-3.5 py-2 rounded-lg bg-zinc-900 border border-zinc-800 flex flex-col">
                            <span className="text-[10px] uppercase font-semibold text-zinc-400">{stat.label}</span>
                      ...
                      <tbody className="divide-y divide-zinc-800/60 bg-zinc-900/30">
                        {(() => {
                          const categoryPriority: Record<HostCategory, number> = {
                            compromised: 0, benignRegular: 1, benignBursty: 2, plainBenign: 3,
                          };
                          ...
                          return sorted.map((row) => {
                            ...
                            const categoryBadge = {
                              compromised: <span className="inline-flex items-center px-2 py-0.5 rounded-md text-[11px] font-medium bg-red-500/10 text-red-400 border border-red-500/20">Compromised</span>,
                              benignRegular: <span className="inline-flex items-center px-2 py-0.5 rounded-md text-[11px] font-medium bg-orange-500/10 text-orange-400 border border-orange-500/20">Benign-Regular</span>,
                              benignBursty: <span className="inline-flex items-center px-2 py-0.5 rounded-md text-[11px] font-medium bg-blue-500/10 text-blue-400 border border-blue-500/20">Benign-Bursty</span>,
                              plainBenign: <span className="inline-flex items-center px-2 py-0.5 rounded-md text-[11px] font-medium bg-zinc-800 text-zinc-300 border border-zinc-700/60">Plain Benign</span>,
                            }[category];

                            return (
                              <tr key={row.hostId} className="hover:bg-zinc-800/40 transition-colors">
                                <td className="py-2.5 px-4 font-mono font-semibold text-zinc-200">{row.hostId}</td>
                                <td className="py-2.5 px-4">{categoryBadge}</td>
                                <td className="py-2.5 px-4 text-right font-mono text-zinc-300">{row.phase1Score.toFixed(4)}</td>
                                <td className="py-2.5 px-4 text-right font-mono text-zinc-300">{row.phase2Score.toFixed(4)}</td>
                                <td className="py-2.5 px-4 text-right font-mono font-bold text-zinc-100">{row.fusedScore.toFixed(4)}</td>
