...
export function KineticMatrix({
    ...
    const shouldShowControls = showControls ?? !hasChildren;
    const shouldShowTitle = showTitle ?? !hasChildren;
    ...
    // Global window pointer listeners for seamless background interactivity
    useEffect(() => {
        ...
        const handleWindowDown = (e: MouseEvent) => {
            const container = containerRef.current;
            if (!container) return;
        ...
        window.addEventListener('mousedown', handleWindowDown, { passive: true });
        window.addEventListener('mouseup', handleWindowUp, { passive: true });

        return () => {
            window.removeEventListener('mousemove', handleWindowMove);
            window.removeEventListener('mousedown', handleWindowDown);
    ...
    return (
        <div
            ...
            className={cn(
                "group relative flex h-full w-full select-none flex-col justify-between overflow-hidden bg-neutral-50 transition-colors duration-700 dark:bg-zinc-950",
                className
            )}
        >
            ...
            <canvas
                ref={canvasRef}
                style={{ opacity }}
                className={cn("absolute inset-0 block h-full w-full pointer-events-none z-0", canvasClassName)}
            />
